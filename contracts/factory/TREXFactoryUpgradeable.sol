...
contract TREXFactoryUpgradeable is ITREXFactory, OwnableUpgradeable {
    ...
    /**
     * @notice Initializes the TREXFactoryUpgradeable contract
     * @param implementationAuthority_ The address of the implementation authority contract
     * @param idFactory_ The address of the Identity Factory used to deploy token OIDs
     * @dev This function can only be called once during contract deployment
     */
    function initialize(address implementationAuthority_, address idFactory_) external initializer {
        __TREXFactoryUpgradeable_init(implementationAuthority_, idFactory_);
    }
    ...
    external override onlyOwner {
        ...
        AgentRoleUpgradeable(address(ir)).addAgent(address(token));
        for (uint256 i = 0; i < (_tokenDetails.irAgents).length; i++) {
            AgentRoleUpgradeable(address(ir)).addAgent(_tokenDetails.irAgents[i]);
        }
        for (uint256 i = 0; i < (_tokenDetails.tokenAgents).length; i++) {
            AgentRoleUpgradeable(address(token)).addAgent(_tokenDetails.tokenAgents[i]);
        }
        ...
        (OwnableUpgradeable(address(token))).transferOwnership(_tokenDetails.owner);
        (OwnableUpgradeable(address(ir))).transferOwnership(_tokenDetails.owner);
        (OwnableUpgradeable(address(tir))).transferOwnership(_tokenDetails.owner);
        (OwnableUpgradeable(address(ctr))).transferOwnership(_tokenDetails.owner);
        (OwnableUpgradeable(address(mc))).transferOwnership(_tokenDetails.owner);
    ...
    // *************************************
    /**
     * @dev Initializes the TREXFactoryUpgradeable contract.
     * @param implementationAuthority_ The address of the implementation authority.
     * @param idFactory_ The address of the ID factory.
     */
    function __TREXFactoryUpgradeable_init(
        address implementationAuthority_,
        address idFactory_
    ) internal onlyInitializing {
        __Ownable_init(msg.sender);
        __TREXFactoryUpgradeable_init_unchained(implementationAuthority_, idFactory_);
    }

    /**
     * @dev Unchained initializer for the TREXFactoryUpgradeable contract.
     * @param implementationAuthority_ The address of the implementation authority.
     * @param idFactory_ The address of the ID factory.
     */
    function __TREXFactoryUpgradeable_init_unchained(
        address implementationAuthority_,
        address idFactory_
    ) internal onlyInitializing {
